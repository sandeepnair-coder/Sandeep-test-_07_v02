...
function launchBattleCardGeneration(brandName, category, segment) {
  ...
  if (prog) {
    ...
    prog.offsetHeight;
    prog.style.animation = 'load 12s ease-in-out forwards';
  }
  ...
  var stepTimings = [[2, 1500], [3, 4000], [4, 7000], [5, 10000]];
  var stepTimers = stepTimings.map(function(pair) {
    return setTimeout(function() {
      var prev = document.getElementById('bstep' + (pair[0] - 1));
      var cur = document.getElementById('bstep' + pair[0]);
...
async function generateBattleCardData(brandName, category, segment) {
  ...
  try {
    ...
    showToast('info', 'Using template data', 'AI generation unavailable. Showing personalised template for ' + brandName + '.', 5000);
  }
}
